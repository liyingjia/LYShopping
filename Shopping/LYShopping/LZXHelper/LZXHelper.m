...
//获取时间差
+ (NSString *)intervalSinceNow:(NSString *)PreviousDate laterDate:(NSString *)laterDate
{
    //把时间字符串 转化为 时间戳
    ...
    NSTimeInterval cha = 0;
    NSString *timeString=@"";
    if ([laterDate isEqualToString:@""]) {
        //获取当前时间
    ...
    if (cha/3600<1) {
        ...
        timeString=[NSString stringWithFormat:@"剩%@分钟", timeString];
    }
    if (cha/3600>1&&cha/86400<1) {
        timeString = [NSString stringWithFormat:@"%f", cha/3600];
        timeString = [timeString substringToIndex:timeString.length-7];
        timeString=[NSString stringWithFormat:@"剩%@小时", timeString];
    }
    if (cha/86400>1)
    {
        timeString = [NSString stringWithFormat:@"%f", cha/86400];
        timeString = [timeString substringToIndex:timeString.length-7];
        timeString=[NSString stringWithFormat:@"剩%@天", timeString];
    }
    return timeString;
}
